...
/*
Run the clockwise astar algorithm. The resulting path is saved in paths
so each node knows the previous node and the path can be traced back.
Returns the path length.
*/
static int run_clockwise_pathfind(MemoryArena *arena, float *weights, int *paths, int w, int h, int start, int goal, int origin, int large)
{
    float weight_baseline = find_min(weights, w * h);
    ...
    int origin_row = origin / w;
    int origin_col = origin % w;
    ...
    while (nodes_to_visit->size > 0)
    {
        ...
        /*
        Since we're only going clockwise, only 3 directions can be checked based on which quadrant
        the current node (x, y) is in relative to the origin (i, j).
        ...
        x == i, y > j: up_right, right, down_right
        x > i, y > j: right, down_right, down
        x > i, y == j: down_right, down, down_left
        x > i, y < j: down, down_left, left
        x == i, y < j: down_left, left, up_left
        x < i, y < j: left, up_left, up
        x < i, y == j: up_left, up, up_right
        ...
        However, the directions here refer to locations in the numpy array, not on the map. After plotting the various
        ...
        */
        ...
        nbrs[UP_LEFT] = -1;  // ((row > 0 && col > 0) && (row >= origin_row && col <= origin_col)) ? cur.idx - w - 1 : -1;
        nbrs[UP] = ((row > 0) && (col < origin_col)) ? cur.idx - w : -1;
        nbrs[UP_RIGHT] = -1;  // ((row > 0 && col + 1 < w) && (row <= origin_row && col <= origin_col)) ? cur.idx - w + 1 : -1;
        nbrs[LEFT] = ((col > 0) && (row > origin_row)) ? cur.idx - 1 : -1;
        nbrs[RIGHT] = ((col + 1 < w) && (row < origin_row)) ? cur.idx + 1 : -1;
        nbrs[DOWN_LEFT] = -1;  // ((row + 1 < h && col > 0) && (row >= origin_row && col >= origin_col)) ? cur.idx + w - 1 : -1;
        nbrs[DOWN] = ((row + 1 < h) && (col > origin_col)) ? cur.idx + w : -1;
        nbrs[DOWN_RIGHT] = -1;  // ((row + 1 < h && col + 1 < w) && (row <= origin_row && col >= origin_col)) ? cur.idx + w + 1 : -1;
...
/*
Exported function to run clockwise astar. Test thingy for cannon walling
Takes in grid weights, dimensions of the grid, requested start and end, and origin.
*/

static PyObject *clockwise_astar(PyObject *self, PyObject *args)
{
    PyArrayObject *weights_object;
    int h, w, start, goal, origin, large, smoothing;

    if (!PyArg_ParseTuple(args, "Oiiiiiii", &weights_object, &h, &w, &start, &goal, &origin, &large, &smoothing))
    {
        return NULL;
    }
...
static int flood_fill_overlord(MemoryArena *arena, uint8_t *heights, uint8_t *point_status, int grid_width, int grid_height, int x, int y, uint8_t target_height, uint8_t replacement, KeyContainer *current_set)
{
    ...
    while (nodes_to_visit->size > 0)
    {
        ...
        if (point_status[key] & IN_CURRENT_SET)
            continue;
...
static void chokes_solve(uint8_t *point_status, float *border_weights, uint8_t *walkable, ChokeLines *choke_lines, int w, int h, int x, int y, int x_start, int y_start, int x_end, int y_end)
{
    ...
    if (point_status[w * y + x] & BORDER)
    {
        ...
        for (int ynew = ymin; ynew < ymax; ++ynew)
        {
            ...
            {
                if (!(point_status[w * ynew + xnew] & BORDER))
                    continue;

                float flight_distance = euclidean_distance(x, y, xnew, ynew);

                if (flight_distance > choke_distance || flight_distance < 2.0f)
                    continue;
...
static PyObject *get_map_data(PyObject *self, PyObject *args)
{
    ...
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
        {
            ...
            if (walkable[w * y + x] == 0)
            {
                ...
                if (walkable[nbr1] || walkable[nbr2] || walkable[nbr3] || walkable[nbr4] || walkable[nbr5] || walkable[nbr6] || walkable[nbr7] || walkable[nbr8])
                {
                    choke_weights[w * y + x] = 1.0f;
            ...
            for (int d = 0; d < 4; ++d)
            {
                ...
                if (walkable[w * y1 + x1] == 1 || walkable[w * y2 + x2] == 0)
                    continue;
    ...
    for (int y = y_start; y < y_end; ++y)
    {
        ...
        {
            if (point_status[w * y + x] & CLIMBABLE && (point_status[w * y + x + 1] & CLIMBABLE || point_status[w * y + x - 1] & CLIMBABLE || point_status[w * (y + 1) + x] & CLIMBABLE || point_status[w * (y - 1) + x] & CLIMBABLE))
            {
